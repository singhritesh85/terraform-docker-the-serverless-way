...
data "aws_iam_policy_document" "ecs_efs_infrastructure_policy" {
  statement {
    effect = "Allow"
    actions = [
...
resource "aws_iam_policy" "ecs_efs_infrastructure_policy" {
  name        = "CustomAmazonECSInfrastructureRolePolicyForEFS"
  description = "Allows ECS tasks to manage EFS File Systems"
  policy      = data.aws_iam_policy_document.ecs_efs_infrastructure_policy.json
}
...
resource "aws_iam_role" "ecs_efs_role" {
  ...
  description        = "IAM role for ECS infrastructure with efs policy"
}

resource "aws_iam_role_policy_attachment" "ecs_efs_attachment" {
  role       = aws_iam_role.ecs_efs_role.name
  policy_arn = aws_iam_policy.ecs_efs_infrastructure_policy.arn
}
...
resource "aws_efs_mount_target" "mysql_efs" {
...
resource "aws_ecs_service" "ecs_service_mysql" {
  ...
  depends_on = [aws_iam_role_policy_attachment.ecs_efs_attachment, aws_iam_policy.ecs_efs_infrastructure_policy]
}
...
##################################################### Autoscale MySQL Containers ######################################################

resource "aws_appautoscaling_target" "mysql_to_target" {
  max_capacity = 3
  min_capacity = 1
  resource_id = "service/${aws_ecs_cluster.ecs_cluster.name}/${aws_ecs_service.ecs_service_mysql.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace = "ecs"
}

resource "aws_appautoscaling_policy" "mysql_to_memory" {
  name               = "mysql-container-to-memory"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.mysql_to_target.resource_id
  scalable_dimension = aws_appautoscaling_target.mysql_to_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.mysql_to_target.service_namespace
...
resource "aws_appautoscaling_policy" "mysql_to_cpu" {
  name               = "mysql-container-to-cpu"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.mysql_to_target.resource_id
  scalable_dimension = aws_appautoscaling_target.mysql_to_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.mysql_to_target.service_namespace
